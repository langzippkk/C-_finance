...
ColumnVector Find_Basic_Solution(Matrix A, ColumnVector y)
{
	...
	// Use sample code 2 from Professor R.S. Sreenivas.
}
		...
		do
		{
			...
			for (int i = 0; i < A.ncols(); i++)
			{
				if (did_i_pick_this_col[i])
				{
					...
					// I put this column in C;
					//note that B here is trim_A transpose; 
					B.push_back(C);
					C.clear();
					//I want to clear C everytime, since I just want B;
				}
			}
			...
			// check rank condition of selected columns of A
			...
			//  put the value of B to matrix reduced_A;
...

			{
				...
				// vector-of-vectors...
				int counterofzero = 0, counterofsol = 1;
				while (counterofzero < A.ncols())
				{
					if (did_i_pick_this_col[counterofzero])
					{
						temp.push_back(soln(counterofsol));
						counterofsol++;
						// If I picked this column, there is a solution corresponding to 
                        // this column, I insert this solution. Else I insert 0.
					}
					else{
						temp.push_back(0);
					}
				...
				// solve all the possible solution and insert zero and then pushback it into 
				// vector of vector, solution_set.
				solution_set.push_back(temp);  // just keep the solution_Set and clear other.
				temp.clear();
			}
			B.clear();
...
{
	Matrix solution(A.ncols(), (int)solution_set.size());
	...
	int rank_of_solution_set = Rank(solution);
	{
		...
		do
		{
			...
			vector <double> C;
			// Discussed with Yufei Gao in changing columns to rows here.
...
void process_data(char* argv[])
{
	...
	if (input_file.is_open()) // If the input file exists in the local directory
	{
		...
		// check if there is a solution, if "yes" then compute general soln
